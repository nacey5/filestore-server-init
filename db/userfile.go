...
func OnUserFileUploadFinished(username, filehash, filename string, filesize int64) bool {
	stmt, err := mydb.DBConn().Prepare("insert ignore into tbl_user_file(`user_name`,`file_sha1`,`file_name`,`filesize`,`upload_at`)" +
		"values (?,?,?,?,?)")
	if err != nil {
		return false
	}
...
// QueryUserFileMetas todo 批量获取用户文件信息 todo 要进行gin进行改造
